#!/usr/bin/env node
// Validate async custom onDiffHunkAction patch/stash interception flow.
...
const cases = [
  {
    name: 'custom-patch-revert-lower',
    fixture: 'inserted',
    side: 'lower',
    action: 'revert',
    expected: {
    ...
    expectedPatchLogPrefix: 'PATCH FLOW REVERT lower |',
  },
  {
    name: 'custom-patch-stage-upper',
    fixture: 'deleted',
    side: 'upper',
    action: 'stage',
    expected: {
    ...
    expectedPatchLogPrefix: 'PATCH FLOW STAGE upper |',
  },
]
...
async function runCase(page, scenario) {
  ...
  try {
    await invokeDiffTestApi(page, 'setHunkActionMode', 'custom-patch')
...
async function run() {
  ...
  try {
    ...
    if (!ok) {
      console.error('Custom diff hunk patch-flow validation failed; recent Vite logs:\n')
