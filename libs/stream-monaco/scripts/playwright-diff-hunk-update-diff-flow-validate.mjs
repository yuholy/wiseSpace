#!/usr/bin/env node
// Validate async custom onDiffHunkAction server updateDiff(...) flow.
...
const cases = [
  {
    name: 'custom-update-diff-revert-lower',
    fixture: 'inserted',
    side: 'lower',
    action: 'revert',
    expected: {
    ...
    expectedDiffCount: 0,
    expectedActionLogPrefix: 'REVERT lower |',
    expectedPatchLogPrefix: 'UPDATE DIFF FLOW REVERT lower |',
  },
  {
    name: 'custom-update-diff-stage-upper',
    fixture: 'deleted',
    side: 'upper',
    action: 'stage',
    expected: {
    ...
    expectedDiffCount: 0,
    expectedActionLogPrefix: 'STAGE upper |',
    expectedPatchLogPrefix: 'UPDATE DIFF FLOW STAGE upper |',
  },
]
...
async function runCase(page, scenario) {
  ...
  try {
    await invokeDiffTestApi(page, 'setHunkActionMode', 'custom-update-diff')
    ...
    const patchFlowLogs = await invokeDiffTestApi(page, 'getPatchFlowLogs')
    const mode = await invokeDiffTestApi(page, 'getHunkActionMode')
    const firstActionLog = actionLogs[0] ?? null
    const firstPatchLog = patchFlowLogs[0] ?? null
    const ok =
      mode === 'custom-update-diff'
      && after.original === scenario.expected.original
      && after.modified === scenario.expected.modified
      && summary.diffCount === scenario.expectedDiffCount
      && typeof firstActionLog === 'string'
      && firstActionLog.startsWith(scenario.expectedActionLogPrefix)
      && typeof firstPatchLog === 'string'
      && firstPatchLog.startsWith(scenario.expectedPatchLogPrefix)
    ...
    const patchFlowLogs =
      await invokeDiffTestApi(page, 'getPatchFlowLogs').catch(() => [])
    const mode =
      await invokeDiffTestApi(page, 'getHunkActionMode').catch(() => null)
...
async function run() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => typeof window.__streamMonacoDiffTestApi?.setHunkActionMode === 'function',
      { timeout: 20000 },
    )
    ...
    await invokeDiffTestApi(page, 'setHunkActionMode', 'default')
    await browser.close()
    ...
    if (!ok) {
      console.error(
        'Custom diff hunk updateDiff-flow validation failed; recent Vite logs:\n',
      )
