...
> - 这份文档作为优化 backlog 和进度状态的单一入口
> - 中文版是主维护版本，状态更新以本文档为准
> - 英文版作为同步镜像，内容应尽量保持一致
> - 每完成一项优化，都要在这里更新状态
> - 如果一项工作只完成了一部分，要补充备注并拆分后续子项
>
> 状态说明：
> - `pending`：未开始
> - `in_progress`：进行中
...
- 范围：
  ...
  - 在 `repo` 和 `command` 层建立规范的 workspace 身份
- 当前进展：
  - 已完成关系模型梳理
  - 已完成 scope audit 和 implementation checklist
  - 已完成 schema 草案与迁移建议
  - 已完成代码改动地图，已进入 migration / entity 实施
  - 已完成 workspace identity foundation migration
  - 已新增 `workspaces` entity，并给相关 entity 补齐 `workspace_id`
  - 已在 `repo / command` 层接通 `conversation / agent / stored file` 的 `workspace_id` 主链路
- 来源：
...
- 状态：`pending`
- 目标：把 `skills`、`MCP`、`tools`、`external connectors`、未来的 UI contribution 收口到统一模型里
- 范围：
...
- 状态：`done`
- 结果：
  - 产出了 `docs/research/COMPETITOR_DECISION_ROADMAP.md`

### C. Workspace 范围审视

- 状态：`done`
- 结果：
  - 产出了 `docs/planning/workspace-scope-audit.md`

### D. Workspace 实施清单

- 状态：`done`
- 结果：
  - 产出了 `docs/planning/workspace-implementation-checklist.md`
...
- 状态：`done`
- 结果：
  - 产出了 `docs/planning/workspace-schema-draft.md`

### H. Workspace 代码改动地图

- 状态：`done`
- 结果：
  - 产出了 `docs/planning/workspace-code-change-map.md`

### I. 自动工作空间名称可读性优化

- 状态：`done`
- 结果：
  - 自动生成的 agent workspace 在输入区不再直接展示 `conv-*` 这类技术目录名
...
- 状态：`done`
- 结果：
  - 新创建的默认 agent workspace 目录会优先使用会话标题生成可读目录名
  - 首条消息生成标题后，会尝试把旧的 `conv-*` 默认目录迁移到可读目录名
