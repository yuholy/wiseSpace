...
- Chat and settings:
  - Added multimodal fallback settings under Default Models and surfaced a lightweight â€œvision fallback usedâ€?display tag in chat output.
  - Normalized malformed/repeated `<think>` content in assistant rendering and delayed heavy markdown re-rendering when returning to chat, reducing visible stalls and repeated â€œthinking completeâ€?blocks.
